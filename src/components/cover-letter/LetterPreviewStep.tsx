...
const LetterPreviewStep: React.FC<LetterPreviewStepProps> = ({
  ...
  const insertKeyword = (keyword: string) => {
    // Insert the keyword at the current cursor position or at the end
    const textarea = document.getElementById('letter-content') as HTMLTextAreaElement;
    if (textarea) {
      const cursorPos = textarea.selectionStart;
      const textBefore = letterContent.substring(0, cursorPos);
      const textAfter = letterContent.substring(cursorPos);
      
      setLetterContent(`${textBefore}${keyword}${textAfter}`);
      
      // Focus the textarea and set cursor position after the inserted keyword
      setTimeout(() => {
        textarea.focus();
        textarea.setSelectionRange(cursorPos + keyword.length, cursorPos + keyword.length);
      }, 0);
    } else {
      // If textarea not found, just append to end
      setLetterContent(prev => `${prev} ${keyword}`);
    }
  };
